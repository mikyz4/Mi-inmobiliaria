document.addEventListener('DOMContentLoaded', function() {
    ...
    if (menuToggle && sidebar) { menuToggle.addEventListener('click', () => sidebar.classList.add('open')); }
    ...
    // --- LÓGICA DE MENÚ Y ESTADO DEL USUARIO ---
    ...
    // --- PROTEGER PÁGINAS PRIVADAS Y CARGAR DATOS ---
    const inicializarPagina = async () => {
        ...
        if (privatePages.includes(currentPage) && !session) {
            showNotification('Debes iniciar sesión para acceder.', 'error', 2000);
    ...
    // --- RENDERIZADO DE ANUNCIOS (FUNCIÓN COMPARTIDA) ---
    const renderAnuncios = (container, anuncios, tipoRenderizado = 'publico') => {
        container.innerHTML = '';
        if (!anuncios || anuncios.length === 0) {
            let mensaje = 'No se encontraron anuncios con estos criterios.';
            if(tipoRenderizado === 'misAnuncios') mensaje = 'Aún no has publicado ningún anuncio. <a href="Anuncio.html" style="color: var(--accent-color);">¡Publica el primero!</a>';
            if(tipoRenderizado === 'admin') mensaje = 'No hay ningún anuncio en la plataforma.';
            container.innerHTML = `<p style="text-align:center; width:100%;">${mensaje}</p>`;
            return;
        }
        ...
        anuncios.forEach(anuncio => {
            ...
            card.innerHTML = `
                ...
                <div class="anuncio-card-content">
                    ...
                    ${tipoRenderizado === 'misAnuncios' || tipoRenderizado === 'admin' ? `
    ...
    // --- LÓGICA PARA VER ANUNCIOS PÚBLICOS ---
    const cargarAnunciosPublicos = () => {
        ...
        const aplicarFiltros = () => {
            ...
            renderAnuncios(anunciosContainer, anunciosFiltrados, 'publico');
        };
        ...
        if(resetFiltrosBtn) {
            resetFiltrosBtn.addEventListener('click', () => {
                document.querySelectorAll('#filtros-wrapper input, #filtros-wrapper select').forEach(el => el.value = el.tagName === 'SELECT' ? 'todos' : '');
                aplicarFiltros();
            });
        }
        cargarDesdeSupabase();
    ...
    const cargarMisAnuncios = async () => {
        ...
        if (!misAnunciosContainer) return;
        
        misAnunciosContainer.innerHTML = '<p style="text-align:center; width:100%;">Cargando tus anuncios...</p>';
        const { data: { user } } = await supabaseClient.auth.getUser();
        if (!user) return;
        try {
            const { data, error } = await supabaseClient.from('anuncios').select('*').eq('user_id', user.id).order('created_at', { ascending: false });
            if (error) throw error;
            renderAnuncios(misAnunciosContainer, data, 'misAnuncios');
        } catch (error) {
    ...
    const cargarTodosLosAnuncios = async () => {
        ...
        if (!adminAnunciosContainer) return;
        ...
        try {
            ...
            if (error) throw error;
            renderAnuncios(adminAnunciosContainer, data, 'admin');
        } catch (error) {
    ...
    // --- LÓGICA GLOBAL DE EVENTOS (MODALES, FAVORITOS, EDITAR, BORRAR) ---
    ...
    if (mainContent) {
        mainContent.addEventListener('click', async (e) => {
            ...
            if (favoriteBtn) {
                ...
                const anuncioId = deleteBtn.dataset.id;
                const isAdmin = !!document.getElementById('adminAnunciosContainer');
                const confirmMessage = isAdmin ? 'ADMIN: ¿Seguro que quieres borrar este anuncio?' : '¿Estás seguro de que quieres borrar este anuncio?';
                ...
                const anuncioId = editBtn.dataset.id;
                const editModal = document.getElementById('editModal');
                const { data, error } = await supabaseClient.from('anuncios').select('*').eq('id', anuncioId).single();
                if (error) {
                    showNotification('Error al cargar los datos del anuncio.', 'error');
                    return;
                }
                ...
                if (!modal) return;
                const anuncioId = parseInt(card.dataset.anuncioId);
                const { data: anuncio, error } = await supabaseClient.from('anuncios').select('*').eq('id', anuncioId).single();
    ...
    if (anuncioForm) {
        ...
        anuncioForm.addEventListener('submit', async function(event) {
            ...
            try {
                ...
                showNotification('¡Anuncio enviado con éxito!', 'success', 3000);
    ...
    // --- FUNCIÓN PRINCIPAL PARA CARGAR EL CONTENIDO DE LA PÁGINA ---
    const cargarContenidoPagina = () => {
