document.addEventListener('DOMContentLoaded', () => {
  ...
  // =================================================================
  // LÓGICA SOLO PARA LA PÁGINA DE INICIO (index.html)
  // =================================================================
  if (document.getElementById('hero')) {
    ...
    // --- Lógica de Testimonios (ESTÁTICA, SIN CARRUSEL) ---
    ...
    if (newsGrid && newsModal) {
        ...
        newsModal.querySelector('.close-button')?.addEventListener('click', () => newsModal.classList.remove('active'));
    }
  ...
  if (anuncioForm) {
      anuncioForm.addEventListener('submit', (e) => {
          ...
          fieldsToValidate.forEach(field => document.getElementById(field.errorDiv).textContent = '');
  ...
  if (anunciosContainer) {
    ...
    const mostrarAnuncios = (anuncios) => {
        ...
        anuncios.forEach(anuncio => {
            ...
            // El código anterior estaba incompleto aquí. Esta es la versión correcta.
            card.innerHTML = `
              <img src="${anuncio.imagenes[0]}" alt="${anuncio.titulo}">
    ...
    anuncioModal?.querySelector('.close-button')?.addEventListener('click', () => anuncioModal.classList.remove('active'));
    ...
    const aplicarFiltrosYOrdenar = () => {
        ...
        let anunciosFiltrados = anunciosData.filter(anuncio => 
            (tipo === 'todos' || anuncio.tipo === tipo) &&
            (anuncio.habitaciones >= habMin && anuncio.habitaciones <= habMax) &&
            (anuncio.banos >= banosMin && anuncio.banos <= banosMax) &&
            (anuncio.superficie >= superficieMin) &&
            (anuncio.precio <= precioMax)
        );
