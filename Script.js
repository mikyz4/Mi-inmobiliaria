        ...
        try {
            ...
            anuncioForm.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    // AÑADIMOS LA LÍNEA PARA LA PÁGINA DE "GRACIAS"
                    finalFormData.append('_next', 'https://selettas.netlify.app/gracias.html');

                    uploadStatus.textContent = 'Imágenes subidas. Enviando formulario...';
                    
                    const formSubmitResponse = await fetch("https://formsubmit.co/miky.tv098@gmail.com", {
                        method: 'POST',
                        body: finalFormData
                    });

                    // Si la redirección funciona, el usuario no verá esto, pero es una buena práctica
                    if (formSubmitResponse.ok) {
                        window.location.href = 'https://selettas.netlify.app/gracias.html';
                    } else {
                        throw new Error('El envío a FormSubmit falló.');
                    }
